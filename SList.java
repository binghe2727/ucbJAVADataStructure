...
public class SList {
  ...
  public void squish() {
	  ...
	  // Object currentObj=head.next.item; changing by using equal
	  //Object currentObj=head.next.item;
 	  //SListNode node=new SListNode(head.next.item,head.next.next);//for seeing the whole line
 	  
 	  SListNode node=head.next;//for seeing the whole line

 	  
	  //SListNode node = head.next;
 	  //head.next=node;
 	  //SListNode node1=new SListNode(head.item,head.next);
 	  
 	  SListNode node1=head;

 	  
 	  //head.next=node1;
 	  //head.next=node1;
	  // we can use the h directly. not possible use the indexing
	  //SListNode index=new SListNode();
	  //while(node!=null)
 	  while((node)!=null){
		  if(node.item.equals(node1.item)){
			  node1.next=node.next;
			  node=node.next;
			  this.size--;
			  
			  //this.size--;//forget!!! attention please.
		  }
		  else{
			  //this.size--;
			  //SListNode node_swap=node1.next;
			  //SListNode node1=new SListNode(node.next.item);
			  //node_swap=node1;
			  //nodeget=node;
			  //node=node.next;
			  //node1.next=node.next;
  ...
  public void twin() {
	...
	/*  SListNode currentNode=head.next;
	  int large=this.size;
	  for(int i=1; i<=large;i++){
		  SListNode node1=new SListNode(node.item,node.next);
		  node.next=node1;
		  node=node.next.next;
	  }*/
