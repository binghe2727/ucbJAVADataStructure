...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    // blank lines or lines already present in this file.)
	  ...
	  int currentIndex=0;
	  int updatingIndex=0;
	  //int[] show= new int[ints.length];//thinking of the size;
	  for(int i=0;i<ints.length;i++){
		  if (ints[i]==ints[currentIndex]){
			  
		  }
		  else{
			  ints[updatingIndex]=ints[currentIndex];
			  updatingIndex++;
			  currentIndex=i;
		  }
	  }
	  ints[updatingIndex]=ints[currentIndex];
	  for(int i=updatingIndex+1;i<ints.length;i++){
		  ints[i]=-1;
	  }
  }
